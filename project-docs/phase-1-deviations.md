...
| Hint line at `900 8.5px` uppercase, place in `#E9622D` (§3 Type, §7) | `700 clamp(10.5px, 0.11vw + 10.07px, 11.5px)`, `.03em`, place in `#8F3A12` | This line carries the price, and it was the least legible type in the product on two axes at once. Uppercase throws away the word shapes you read by and weight 900 closes the counters at exactly the size where they are already collapsing; the tracking is what a run this long needs once it is set in caps. The colour is the design system correcting itself — §3 states plainly that `#E9622D` is 2.73:1 on light grounds and that `#8F3A12` (5.99:1) goes there instead, but the prototype used the bright orange here anyway. Measured on the built page: 2.67:1 before, 5.99:1 now. |
