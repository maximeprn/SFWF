...
| Four sponsor marks at `height: clamp(24px,2.8vw,32px)` | Ten marks at `clamp(34px,4.2vw,46px)` | The festival supplied six more (Coconut Cruisers, Greenhouse, Masterplan Global, Ripple, The Henry, Tropika), four of which are square badges rather than landscape wordmarks. At the handoff's height a badge rendered 25×25 beside a 122px-wide Destileria and read as a dot. The row's `max-width` stays a safety net that never binds at any width — checked at 390, 900 and 1440 — so nothing is squashed, and `object-fit: contain` means a mark that somehow did hit it would letterbox. |
